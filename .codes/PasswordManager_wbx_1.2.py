...
class PasswordManager:
    ...
    def create_widget(self):
        ...
        self.tree.column("pwd",width=140)
    ...
    def input_dialog(self, title, old_site="", old_user="", old_pwd="", old_tags=""):
        ...
        res = {"site":old_site,"user":old_user,"pwd":old_pwd,"tags":old_tags,"ok":False}
        ...
        tk.Label(win,text="标签（可搜索关键词，用逗号分隔）：",bg="#f0f0f0").pack(pady=(6,2))
        e4 = tk.Entry(win,width=45)
        e4.insert(0,old_tags)
        e4.pack()
        ...
        with open(PWD_FILE,"r",encoding="utf-8") as f:
            for line in f.readlines():
                ...
                if len(parts) == 3:
                    ...
                    # 新格式: site|user|pwd|tags
                    site, user, pwd = parts[0], parts[1], parts[2]
                    tags = "|".join(parts[3:])
                else:
                    continue
                hide = "●"*len(pwd)
                self.tree.insert("",tk.END,values=(site,user,hide,tags),tags=(pwd,))
                self.all_data.append([site, user, pwd, tags])
    ...
    def save_all(self):
        ...
        for item in self.tree.get_children():
            ...
            lst.append(f"{val[0]}|{val[1]}|{real}|{val[3]}")
    ...
    def add_item(self):
        ...
        if not ret["ok"]:
            return
        site,user,pwd,tags = ret["site"],ret["user"],ret["pwd"],ret["tags"]
        if not all([site,user,pwd]):
            messagebox.showwarning("提示","网站地址、用户名、密码不能为空")
            return
        hide = "●"*len(pwd)
        self.tree.insert("",tk.END,values=(site,user,hide,tags),tags=(pwd,))
        self.save_all()
    ...
    def edit_item(self):
        ...
        if not sel:
            ...
            return
        old_s,old_u,_,old_t = self.tree.item(sel,"values")
        old_p = self.tree.item(sel,"tags")[0]
        ret = self.input_dialog("修改账号信息",old_s,old_u,old_p,old_t)
        if not ret["ok"]:
            return
        ns,nu,np,nt = ret["site"],ret["user"],ret["pwd"],ret["tags"]
        if not all([ns,nu,np]):
            messagebox.showwarning("提示","网站地址、用户名、密码不能为空")
            return
        self.tree.item(sel,values=(ns,nu,"●"*len(np),nt),tags=(np,))
        self.save_all()
    ...
    def search_item(self, event):
        ...
        self.tree.delete(*self.tree.get_children())
        for site, user, pwd, tags in self.all_data:
            # 搜索范围：网站、用户名、标签
            if keyword in site.lower() or keyword in user.lower() or keyword in tags.lower():
                hide = "●"*len(pwd)
                self.tree.insert("",tk.END,values=(site,user,hide,tags),tags=(pwd,))
